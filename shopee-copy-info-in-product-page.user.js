...
`;
    ...
    const getDiscounts = () => {
        ...
        Object.keys(elem).forEach((keyelem) => {
            if (keyelem.toLowerCase().includes("fiber")) {
                keys.fiber = keyelem;
            } else if (keyelem.toLowerCase().includes("props")) {
                keys.props = keyelem;
            }
        });
        const paths = [
        ...
        function _getValueByPath(obj, pathArray) {
        ...
        return _getValueByPath(elem, paths);
    };
